...
def generate_shipment(shipment_id):
    ...
    inside_temps = [baseline_temp + np.random.normal(0, 1.0) for _ in range(60)]
    ...
    if is_breached:
        ...
        for i in range(failure_point, 60):
            inside_temps[i] = inside_temps[i] + (i - failure_point) * 0.3 + np.random.normal(0, 1.5)
        if random.random() < 0.3:
            recovery_point = random.randint(failure_point + 5, 59)
            for i in range(recovery_point, 60):
                inside_temps[i] -= random.uniform(1, 3)
    else:
        n_spikes = random.randint(1, 5)
        for _ in range(n_spikes):
            spike_idx = random.randint(0, 59)
            inside_temps[spike_idx] += random.uniform(3, 8)
        for i in range(60):
